...
<head>
  ...
  <title>Confirm Password - Dekranasda</title>
...
<body class="h-screen flex relative">
  ...
  <div class="flex-1 flex flex-col justify-center items-center bg-white px-6">
    ...
    <h2 class="text-2xl font-semibold text-coklat mb-2">Confirm your password</h2>
    <p class="text-sm text-gray-600 mb-6 text-center max-w-sm">
      Demi keamanan, masukkan password Anda untuk melanjutkan.
    </p>

    <form method="POST" action="{{ route('password.confirm') }}" class="w-full max-w-sm space-y-4">
      @csrf
